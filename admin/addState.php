...
<body class="sidebar-mini">
            ...
            <div class="col-md-6 grid-margin stretch-card container center_div">
              <div class="card">
                <div class="card-body">
                  ...
                  <form class="forms-sample" method="post" id="AddState">
                    <?php
                    if(isset($_POST["btnsubmit"]))
                    {
                      $result=mysqli_query($conn,"insert into tbl_state (state_name,state_code) values('".$_POST["txtstatename"]."','".$_POST["txtstatecode"]."')")or die(mysql_error($conn));
                      if($result==true)
                      {
                        ?>
                    ...
                    <div class="form-group">
                      ...
                      <input type="text" class="form-control" name="txtstatename" id="txtstatename" placeholder="State name">
                      <p class="error" id="duperror"></p>
                    </div>
                   <div class="form-group">
                      <label for="exampleInputUsername1">State Code</label>
                      <input type="text" class="form-control" name="txtstatecode" id="txtstatecode" placeholder="State code">
                    ...
                    <button type="submit" name="btnsubmit" id="btnsubmit" class="btn btn-primary float-right mr-2">Submit</button>
                     </form>
                </div>
              </div>
            </div>
  ...
  <script>
    ...
    $('#txtstatename').blur(function() {
      //alert("sdf");
       $.ajax({
          url: "statevalidation.php",
          type: 'POST',
          ...
          success: function(result){
                     ...
                     {  
                      ...
                      $("#duperror").html("State Name Already Exist!");
                        //alert("Already Exist");
                     }
                     else
                     {
    ...
    $('#txtstatecode').blur(function() {
      //alert("sdf");
       $.ajax({
          url: "statecodevalidation.php",
          type: 'POST',
          ...
          success: function(result){
                     ...
                     {  
                      $("#btnsubmit").prop('disabled', true);
                      $("#duperrorcode").html("State Code Already Exist!");
                        //alert("Already Exist");
                     }
                     else
                     {
